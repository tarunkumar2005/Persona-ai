...
import { PIYUSH_GARG_SYSTEM_PROMPT } from "@/lib/prompts"
...
export async function POST(request: NextRequest) {
  try {
    const { message, sessionId = "piyush-default" } = await request.json()
    ...
    const messages = [
      { role: "system" as const, content: PIYUSH_GARG_SYSTEM_PROMPT },
      ...history.map((msg) => ({
